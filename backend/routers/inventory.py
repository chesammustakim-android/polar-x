...
def require_inventory_manager(current_user: models.User = Depends(get_current_user)) -> models.User:
    """Enforces that the user has Inventory / Logistics management authority."""
    role = (current_user.role or "").upper()
    if role not in ["ADMIN", "EXPEDITION_DIRECTOR", "LOGISTICS_OFFICER"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Inventory management requires LOGISTICS_OFFICER, EXPEDITION_DIRECTOR, or ADMIN authority. Your role: {current_user.role}"
        )
    return current_user
...
def get_station_inventory_intelligence(
    ...
    All other operational roles: global operational view (can filter by station_id or get all stations).
    """
    role = (current_user.role or "").upper()
...
def get_donor_recommendations(
    ...
    """
    # All authenticated operational roles can inspect donor intelligence across stations
...
def create_inventory_item(
    ...
    current_user: models.User = Depends(require_inventory_manager),
    db: Session = Depends(get_db)
):
    """
    Register a new inventory resource item.
    Requires LOGISTICS_OFFICER, EXPEDITION_DIRECTOR, or ADMIN authority.
...
def update_inventory_item(
    ...
    """Update item metadata (minimum threshold, category, unit, location, expedition). Requires LOGISTICS_OFFICER, EXPEDITION_DIRECTOR, or ADMIN."""
...
def perform_stock_operation(
    ...
    - LOGISTICS_OFFICER, EXPEDITION_DIRECTOR, ADMIN: global write access.
    - STATION_HEAD: restricted to inventory items located at their assigned station.
    - Other roles: 403 Forbidden.
    """
    role = (current_user.role or "").upper()
    if role not in ["ADMIN", "EXPEDITION_DIRECTOR", "LOGISTICS_OFFICER", "STATION_HEAD"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Stock operations require inventory management authority. Your role: {current_user.role}"
        )
    ...
    if role == "STATION_HEAD":
        assigned_st = crud.get_station(db, current_user.assigned_station_id) if current_user.assigned_station_id else None
        st_name = (assigned_st.name if assigned_st else "").lower()
        first_token = st_name.split()[0] if st_name.split() else ""
        loc = (item.location or "").lower()
        if not st_name or (first_token not in loc and st_name not in loc):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Station Head can only perform stock operations for items located at their assigned station."
            )
